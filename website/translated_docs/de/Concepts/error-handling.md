...
Viele Class Functions von 4D, wie `entity.save()` oder `transporter.send()`, geben ein Objekt *status* zurück. This object is used to store "predictable" errors in the runtime context, e.g. invalid password, locked entity, etc., that do not stop program execution. This category of errors can be handled by regular code.
...
Mit dem Befehl `Method called on error` erfahren Sie den Namen der Methode, die über den Befehl `ON ERR CALL` für den aktuellen Prozess installiert wurde. It is particularly useful in the context of generic code because it enables you to temporarily change and then restore the error-catching method:
