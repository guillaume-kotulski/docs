...
For security reasons, by default, 4D does not respond to REST requests. Si vous souhaitez démarrer le serveur REST, vous devez cocher l'option **Exposer en tant que serveur REST** dans la page "Web/REST" des paramètres de la base pour que les demandes REST soient traitées.
...
Par défaut, le menu affiche **<Anyone>**, ce qui signifie que les accès REST sont ouverts à tous les utilisateurs. Once you have specified a group, only a 4D user account that belongs to this group may be used to [access 4D by means of a REST request](authUsers.md). If an account is used that does not belong to this group, 4D returns an authentication error to the sender of the request.
