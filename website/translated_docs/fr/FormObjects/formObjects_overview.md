...
*   **JSON code in dynamic forms:** Define the properties using JSON. Utilisez la propriété [type](properties_Object.md#type) pour définir le type d'objet puis indiquez ses [propriétés disponibles](properties_Reference.md). Exemple d'un objet bouton :
```
    {
        "type": "button", 
        "style": "bevel", 
