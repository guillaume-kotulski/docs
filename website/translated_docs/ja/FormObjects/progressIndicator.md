...
バーバーショップは [スピナー](spinner.md) のように連続したアニメーションを表示します。 These thermometers are generally used to indicate to the user that the program is in the process of carrying out a long operation. When this thermometer variant is selected, [graphical Scale properties](properties_Scale.md) are not available.
