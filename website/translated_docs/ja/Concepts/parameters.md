...
メソッドからデータを返すこともできます。 以下は、文字列のデータ長を返すビルトインの `Length` コマンドを用いたステートメントです。 The statement puts the value returned by `Length` in a variable called *MyLength*. という変数に値を返します。
...
- `On Drag Over` フォームイベントを受け入れるフォームオブジェクト - `On Drag Over` フォームイベントの結果である $0 パラメーター (倍長整数) は、明確に定義されていなければコンパイラーが型を決定します。 定義する場合は、オブジェクトメソッドの中でおこなう必要があります。 **Note:** The compiler does not initialize the $0 parameter. したがって、`On Drag Over` フォームイベントを使用したら、直ちに $0 を初期化しなければなりません。 たとえば:
```4d
 C_LONGINT($0)
