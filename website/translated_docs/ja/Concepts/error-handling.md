...
このプロジェクトメソッドはカレントプロセスに対して実装 (インストール) され、インタープリターモードかコンパイルモードかにかかわらず、プロセス内で発生するすべてのエラーの際に自動で呼び出されます。 To *install* this project method, you just need to call the `ON ERR CALL` command with the project method name as parameter. たとえば:
...
`ON ERR CALL` コマンドによって実装されたエラー処理メソッドは実行中のデータベースにしか適用されません。 In the case of an error generated by a **component**, the `ON ERR CALL` error-handling method of the host database is not called, and vice versa.
...
(*) 4D automatically maintains a number of variables called **system variables**, meeting different needs. 詳細については [4D ランゲージマニュアル](https://doc.4d.com/4Dv18/4D/18/System-Variables.300-4505547.ja.html) を参照ください。</p>
